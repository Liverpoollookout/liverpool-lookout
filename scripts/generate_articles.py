...
# ARTICLE PROMPTS
BASE_INSTRUCTIONS = """
You are a professional football journalist specialising in Liverpool FC for the 2025-26 season.
...
CRITICAL — CURRENT SQUAD ONLY:
Only write about players who are CURRENTLY at Liverpool FC in the 2025-26 season.
The following players have LEFT Liverpool and must NEVER be referenced as current squad members:
- Trent Alexander-Arnold (left for Real Madrid, summer 2024)
- Joel Matip (left 2024)
- Thiago Alcantara (left 2024)
- James Milner (left 2023)
...
- Naby Keita (left 2023)
- Alex Oxlade-Chamberlain (left 2023)
You may reference departed players only in historical/retrospective articles about past seasons.
The current right-back is Conor Bradley. Do not mention Trent Alexander-Arnold in any current squad or tactical context.
"""
...
PROMPTS = {
        ...
        Context data: {json.dumps(ctx, indent=2)}
        Cover: recent form in the 2025-26 season, role in the team, strengths, areas to improve, statistics context, fan verdict.
        Only write about players currently at Liverpool FC this season.
        Return ONLY valid JSON:
        ...
        Context data: {json.dumps(ctx, indent=2)}
        Cover: potential targets being linked in the current window, reported links, likely fees (use "reported" language), how they would fit at Anfield.
        Focus on transfer activity relevant to the 2025-26 season — summer 2025 window or January 2026 window.
        ...
        Context data: {json.dumps(ctx, indent=2)}
        Cover: Arne Slot's system in the 2025-26 season (4-2-3-1 / 4-3-3 hybrid), pressing triggers, build-up patterns, set-piece routines.
        Reference only current squad members. The right-back role is filled by Conor Bradley.
        ...
        Context data: {json.dumps(ctx, indent=2)}
        Focus on one statistical theme from the current 2025-26 season. Use plausible, contextually appropriate statistics.
        ...
        Context data: {json.dumps(ctx, indent=2)}
        Cover: injury updates, returns from fitness, suspensions, squad rotation plans for the current 2025-26 season.
        Only reference players in Liverpool's current squad.
        Return ONLY valid JSON:
        ...
        Context data: {json.dumps(ctx, indent=2)}
        Choose a famous match, signing, trophy, or moment in Liverpool history.
        Rich storytelling approach. You may reference past players (Gerrard, Rush, Dalglish, Fowler, etc.) in a historical context.
        Return ONLY valid JSON:
        ...
        Context data: {json.dumps(ctx, indent=2)}
        Take a clear, well-argued stance on a topical Liverpool FC debate for the 2025-26 season.
        Structure: hook intro, argument, counter-argument, your verdict.
        Only reference current squad members when discussing the present team.
        Return ONLY valid JSON:
        ...
        Context data: {json.dumps(ctx, indent=2)}
        Cover rising talent from Liverpool's academy, U21s, or loan players in the 2025-26 season.
        Return ONLY valid JSON:
                                                    ...
                                                    "stadium": STADIUM,
                                                            "season": SEASON,
                                                                    "key_players": KEY_PLAYERS,
                                                                            "departed_players_do_not_reference_as_current": DEPARTED_PLAYERS,
                                                                                                                    ...
                                                                                                                    recent_results = results[:3] if results else []
                                                                                                                        ...
                                                                                                                        players_pool = KEY_PLAYERS.copy()
                                                                                                                            random.shuffle(players_pool)
                                                                                                                                ...
                                                                                                                                plan = [
                                                                                                                                        ("match_preview", {**base, "next_match": next_match, "recent_form": recent_results, "focus": "detailed match preview with lineup predictions for 2025-26 season"}),
                                                                                                                                                ("player_spotlight", {**base, "player": players_pool[0], "recent_results": recent_results, "focus": "current 2025-26 season form and contribution"}),
                                                                                                                                                        ("transfer_news", {**base, "focus": "summer 2025 or January 2026 transfer targets and potential incomings", "rumour_tier": "Tier 2-3 links from European clubs"}),
                                                                                                                                                                ("tactical_analysis", {**base, "recent_results": recent_results, "focus": "pressing system, high line, and build-up play under Arne Slot in 2025-26"}),
                                                                                                                                                                        ("stats_analysis", {**base, "focus": "2025-26 Premier League attacking statistics comparison - top 6 clubs", "metric_theme": random.choice(["xG and xGA", "pressing intensity", "goals from set pieces", "clean sheet ratio", "shots on target percentage"])}),
                                                                                                                                                                                ("team_news", {**base, "upcoming": next_match, "focus": "injury and fitness update ahead of next fixture — current 2025-26 squad only"}),
                                                                                                                                                                                        ("historical", {**base, "focus": "famous Liverpool FC moment, signing or match from club history", "era_hint": random.choice(["Shankly era (1959-74)", "Paisley golden age (1974-83)", "Dalglish era", "Gerrard years (2000s)", "Klopp era (2015-2024)"])}),
                                                                                                                                                                                                ("player_spotlight", {**base, "player": players_pool[1], "focus": "xG contribution, progressive carries, and impact on pressing in the 2025-26 season"}),
                                                                                                                                                                                                        ("opinion", {**base, "last_result": last_result, "topic": random.choice([
                                                                                                                                                                                                                    "Is Slot's Liverpool on course for the Premier League title in 2025-26?",
                                                                                                                                                                                                                                "Conor Bradley: Is he now Liverpool's best right-back in years?",
                                                                                                                                                                                                                                            "Why Arne Slot deserves more credit for Liverpool's 2025-26 form",
                                                                                                                                                                                                                                                        "The case for promoting an academy player to the first team this season",
                                                                                                                                                                                                                                                                    "Liverpool's summer 2025 signings — hit or miss so far?",
                                                                                                                                                                                                                                                                                "Can Liverpool compete on all four fronts in 2025-26?",
                                                                                                                                                                                                                                                                                            "Mohamed Salah's contract situation — what's the latest and what does it mean for Liverpool?",
                                                                                                                                                                                                                                                                                                    ])}),
                                                                                                                                                                                                                                                                                                            ("youth_academy", {**base, "focus": "Liverpool Academy graduates and current U21 standouts in 2025-26", "player_hint": random.choice(["Ben Doak", "Bobby Clark", "Luke Chambers", "James McConnell", "Trey Nyoni", "Jayden Danns"])}),
                                                                                                                                                                                                                                                                                                                ]
